...
#include <esp_sleep.h>     // Include ESP32 library for deep sleep functions
...
//        display.rtcSetTime(00, 05, 00); // 24H mode, ex. 13:30:00
        //  display.setDate(weekday, day, month, yr);
//        display.rtcSetDate(2, 1, 14, 2025); // 1 for Monday, ex. Monday, 13JAN2025.

        // display.rtcSetEpoch(1589610300); // Or use epoch for setting the time and date
 //   } 

    //display.rtcSetEpoch(1736813580);
    ...
    //display.rtcSetAlarmEpoch(display.rtcGetEpoch() + 10, RTC_ALARM_MATCH_SS); // Set RTC alarm 10 seconds from now
